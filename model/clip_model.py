...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = inputs[0]
        attn, attn_weight = self.attention(self.ln_1(x))
        x = x + attn
...
class CLIP(nn.Module):
    ...
    def forward(self, image=None, text=None):
        if image is None and text is None:
            raise ValueError('Both image and text are None.')
        ret = EasyDict()
        if image is not None:
            image_features, img_attn = self.encode_image(image)
            ret.update(img_feats=image_features, img_attn=img_attn)
        if text is not None:
            text_features, text_attn = self.encode_text(text)
            ret.update(text_feats=text_features, text_attn=text_attn)
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    print('Resized visual position embedding from size: {} to size: {} with height:{} width: {}'.format(posemb.shape,
                                                                                                       posemb_new.shape,
                                                                                                       hight,
                                                                                                       width))
...
def resize_text_pos_embed(posemb, context_length, interpolation: str = 'linear', antialias: bool = False):
    old_num_pos, old_width = posemb.shape[0], posemb.shape[1]
    posemb = posemb.reshape(1, old_num_pos, old_width).permute(0, 2, 1)
    ...
    posemb = posemb.permute(0, 2, 1)[0]
    print('Resized text position embedding from size:[{}, {}] to size: {}'.format(old_num_pos, old_width, posemb.shape))
    return posemb


def convert_weights(model: nn.Module, dtype=torch.float16):
    """Convert applicable model parameters to fp16 or bf16"""
