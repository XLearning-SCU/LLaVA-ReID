...
from model.llava.train.train_utils import safe_save_model_for_hf_trainer
...
class LLaVATrainer(Trainer):
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        # rank0_print(self.state.global_step, self.state.max_steps, self.args.gumbel_temperature, inputs.keys())
        if self.args.gumbel_temperature is not None:
            warmup_steps = self.args.language_model_warm_up_steps
            # rank0_print(warmup_steps, self.state.global_step)
            if self.state.global_step >= warmup_steps:
                start, end = self.args.gumbel_temperature, self.args.gumbel_temperature_end
                temperature = start + (end - start) * self.state.global_step / self.state.max_steps
    ...
    def create_accelerator_and_postprocess(self):
        ...
        if self.is_fsdp_enabled:
            ...
            fsdp_plugin.limit_all_gathers = self.args.fsdp_config.get("limit_all_gathers",
                                                                      fsdp_plugin.limit_all_gathers)
            if is_accelerate_available("0.23.0"):
                ...
                if fsdp_plugin.activation_checkpointing and self.args.gradient_checkpointing:
                    raise ValueError(
                        "The activation_checkpointing in FSDP config and the gradient_checkpointing in training arg " "can't be set to True simultaneously. Please use FSDP's activation_checkpointing logic " "when using FSDP.")
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_vision_tower_lr is not None:
                ...
                rank0_print("Setting Vision Tower lr to {}".format(lr_mapper["vision_tower"]))
            if self.args.mm_selector_lr is not None:
                lr_mapper["selector"] = self.args.mm_selector_lr
            if len(lr_mapper) > 0:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        rank0_print(f"Saving checkpoint to {output_dir}...")
        ...
        output_dir = os.path.join(output_dir, "model_checkpoint")
        if "mm_selector" in model.question_model.model.config.mm_tunable_parts:
            save_model(name="selector",
                       model=model.selector, save_path=output_dir,
                       logger=None, args=None)
            rank0_print("Selector model saved to {}".format(output_dir))
        if "mm_language_model" in model.question_model.model.config.mm_tunable_parts:

            if self.args.lora_enable:
                # state_dict = get_peft_state_maybe_zero_3(model.named_parameters(), self.args.lora_bias)
                # non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(model.named_parameters())
                if self.args.local_rank == 0 or self.args.local_rank == -1:
                    if hasattr(model.question_model.model, "config"):
                        model.question_model.model.config.save_pretrained(output_dir)
                #     if hasattr(model, "generation_config"):
                #         model.generation_config.save_pretrained(self.args.output_dir)
                ...
                model.question_model.model.save_pretrained(output_dir, safe_serialization=False)
            else:
                safe_save_model_for_hf_trainer(trainer=self, output_dir=output_dir)
